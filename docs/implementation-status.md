...
- DOKU onboarding ke SQLite.
- DOKU provider auth test via signed Check Status request.
...
- DOKU `pay create` untuk Checkout.
- DOKU `pay status` untuk Check Status API.
...
- Webhook signature verification untuk DOKU HTTP Notification.
- Webhook parsing event untuk payload Midtrans, Xendit, dan DOKU.
...
rutebayar pay create --provider doku --method checkout --reference rb-doku-001 --amount 15000
