...
2. Buat database E2E baru.
3. Jalankan:
   - `rute-bayar db migrate`
   - `rute-bayar onboard xendit --secret-key "$XENDIT_SECRET_KEY" --webhook-token "$XENDIT_WEBHOOK_TOKEN" --environment sandbox`
   - `rute-bayar webhook serve --addr 127.0.0.1:<port> --environment sandbox`
4. Buka tunnel:
   - `cloudflared tunnel --url http://127.0.0.1:<port>`
5. Di Xendit dashboard sandbox/development, set webhook URL Payment Sessions ke:
   - `https://<public-domain>/webhooks/xendit`
6. Buat payment:
   - `rute-bayar pay create --provider xendit --method payment_link --reference <ref> --amount 15000`
...
8. Verifikasi:
   ...
   - `pay status` sinkron dengan provider.
