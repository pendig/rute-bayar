...
func webhookCommand(ctx context.Context, stdout, stderr io.Writer, args []string) error {
	...
	case "serve":
		...
		mode := fs.String("mode", webhookModeWebhook, "daemon runtime mode: webhook, api, or all")
...
func webhookForwardAdd(ctx context.Context, stdout, stderr io.Writer, args []string) error {
	...
	fs.SetOutput(stderr)
	providerCode := fs.String("provider", "midtrans", "provider code: midtrans, xendit, or doku")
...
func webhookReplayCommand(ctx context.Context, stdout, stderr io.Writer, args []string) error {
	...
	fs.SetOutput(stderr)
	providerCode := fs.String("provider", "", "provider code: midtrans, xendit, or doku")
